...
public class WearWifiMediator implements
    ...
    public class WifiDecision implements EventHistory.Event {
    ...
    public void updateOffBodyState(boolean isOffBody) {
        if (mIsOffBody != isOffBody) {
            ...
            updateWifiState("off body state changed: " + mIsOffBody);
        }
    }
    ...
    private void updateWifiState(String reason) {
        ...
        if (mInHardwareLowPowerMode) {
            ...
            setWifiDecision(false, WifiDecisionReason.OFF_OFF_BODY);
    ...
    private void setWifiEnabled(boolean enable) {
        if (enable == mWifiEnabled) {
            Log.d(TAG, "Not changing wifi state. Wifi state is already "
                    + (enable ? "Enabled" : "Disabled"));
            return;
        }
        mWifiEnabled = enable;
        Log.i(TAG, mWifiEnabled ? "Enabling wifi..." : "Disabling wifi...");
        mWifiManager.setWifiEnabled(mWifiEnabled);
    }
    ...
    private void refreshBackoffStatus() {
        ...
        mWifiLogger.recordWifiState(mWifiEnabled, mWifiConnected, connectivityServiceWantsWifi());
    }
    ...
    public void dump(IndentingPrintWriter ipw) {
        ...
        ipw.printPair("mWifiEnabled", mWifiEnabled);
        ipw.println();
